...
class AINorvanaController:
    def __init__(self):
        ...
        self.start_button = tk.Button(self.root, text="Start AI Control", command=self.start_ai_control)
        self.start_button.pack()
        
        self.interrupt_button = tk.Button(self.root, text="Interrupt", command=self.interrupt_ai_control, state=tk.DISABLED)
    ...
    def generate_image(self, prompt):
        try:
            # 定位并清除提示输入框
            prompt_box = self.wait.until(EC.presence_of_element_located((By.ID, "txt2img_prompt")))
            prompt_box.clear()
            ...
            # 点击生成按钮
            generate_button = self.wait.until(EC.element_to_be_clickable((By.ID, "txt2img_generate")))
